...
#[derive(Fail, Debug)]
#[fail(display = "ratelimiter error: {}", _0)]
...
impl<'a, T: 'a + store::Store> RateLimiter<'a, T> {
    ...
    pub fn rate_limit(&mut self,
                      key: &str,
                      quantity: i64)
                      -> Result<(bool, RateLimitResult), Error> {
        ...
        loop {
            ...
            if i > MAX_CAS_ATTEMPTS {
                return Err(Error(format!("Failed to update rate limit after \
                                                           {} attempts",
                                  MAX_CAS_ATTEMPTS)));
            }
        }
...
mod tests {
    ...
    fn it_handles_rate_limit_update_failures() {
        ...
        let err = Error("Failed to update rate limit after 5 attempts".into());

        assert_eq!(err.compat().description(),
        limiter.rate_limit("foo", 1).unwrap_err().compat().description());
    }
